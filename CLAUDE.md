# Instruksi untuk AI (Claude)

## Wajib Dibaca Sebelum Membantu
- Kamu HARUS membaca seluruh PRD di bawah ini sebelum memberikan saran, membuat komponen, route, atau fitur apapun
- Semua keputusan teknis (naming, struktur folder, UI, fitur) harus mengacu pada PRD ini
- Jika ada yang bertentangan antara instruksi user dan PRD, tanyakan klarifikasi terlebih dahulu
- Jangan membuat fitur, komponen, atau data dummy yang tidak relevan dengan PisangDB

## Konteks Project
- **Nama App:** PisangDB 🍌
- **Tech Stack:** TanStack Start · TypeScript · Tailwind CSS · shadcn/ui · TanStack Query · Drizzle ORM · PostgreSQL · Gemini AI
- **Tujuan:** SaaS tool untuk membuat ephemeral database (PostgreSQL/MySQL/MariaDB) secara instan tanpa instalasi

## Aturan Pengembangan
- Gunakan TanStack Router file-based routing (`src/routes/`)
- Layout app (dengan sidebar) ada di `src/routes/_app.tsx` dan `src/routes/_app/`
- Layout auth (tanpa sidebar) ada di `src/routes/(auth)/`
- Komponen UI menggunakan shadcn/ui dari `src/components/ui/`
- Custom komponen di `src/components/`
- Import alias menggunakan `#/` untuk `src/`
- Formatter dan linter menggunakan Biome (bukan ESLint/Prettier)
- Semua kode harus lolos `pnpm biome check` tanpa error
...
Pilih engine — PostgreSQL, MySQL, atau MariaDB
Pilih durasi — 1 jam sampai 7 hari
Klik "Buat Sandbox"
...
Database pollution — Database dev penuh "tabel zombie" yang lupa dihapus
Tedious local setup — Harus install, config, dan maintain engine di laptop
Cross-engine pain — Beda project beda engine, laptop jadi berat
Migration risk — Takut test migrasi di staging, tapi tidak ada tempat lain
Collaboration friction — Susah share database temporary untuk code review
Learning barrier — Pelajar stuck di instalasi, bukan belajar SQL
Resource waste — Engine DB yang selalu menyala makan RAM
...
Install engine — Download & install PostgreSQL/MySQL/MariaDB (beda OS, beda cara)
Konfigurasi — Set password root, buat user, atur permission
Buat database — createdb atau CREATE DATABASE manual
Setup connection — Tulis connection string di .env, tes koneksi
Troubleshoot — Port conflict, permission denied, service not running, versi tidak cocok
...
Process:
User memilih engine, region, nama, dan durasi
System menentukan target Docker container berdasarkan engine
System menentukan host berdasarkan region ({region}.pisangdb.com)
Generate unique database name: pisang_{user_short_id}_{sandbox_name}_{random_suffix}
Generate unique database username: sb_{random_8_char}
Generate random password (32 char, cryptographic random)
Connect ke container engine yang dipilih sebagai admin
CREATE DATABASE pada container tersebut
CREATE USER — buat akun database baru khusus untuk sandbox ini
GRANT — berikan user tersebut akses hanya ke database sandbox-nya, tidak ke database lain
REVOKE — pastikan user tidak punya akses superuser/root, tidak bisa create database lain
Generate credentials (host, port, username, password, database name)
Simpan metadata ke tabel sandboxes (termasuk db_user dan db_password ter-encrypt)
...
Connect ke container engine yang sesuai sebagai admin
Terminate semua active connections milik db_user sandbox
Execute DROP DATABASE IF EXISTS {db_name}
Execute DROP USER IF EXISTS {db_user} — hapus akun database-nya
Set status → expired
Hapus / soft-delete metadata dari tabel sandboxes
...
Set status → destroying
Tentukan target engine container berdasarkan kolom engine
Connect ke container engine sebagai admin
Terminate semua active connections milik db_user sandbox
Execute DROP DATABASE IF EXISTS {db_name} pada container yang sesuai
Execute DROP USER IF EXISTS {db_user} — hapus akun database sandbox
Set status → expired
Log cleanup event
...
No redundancy — db.pisangdb.com mengulang "db" dari PisangDB, terasa awkward
Multi-region ready — Subdomain langsung menunjukkan lokasi server, user bisa pilih region terdekat
Latency transparency — Developer tahu persis dimana database mereka berada
Scalable — Menambah region baru = menambah subdomain + deploy server baru
DNS Setup (MVP):
